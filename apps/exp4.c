...
int main(int argc, char *argv[]) {
	...
	if (argc!=2) {
		printf("usage: ./scap4 cutoff\n");
		exit(1);
	}
	cutoff=atoi(argv[1]);
	printf("using cutoff %d bytes\n",cutoff);
	...
	//create: using cutoff, 1 second timeout, 1024M mem, fast reassembly, no packets
	sc=scap_create("eth3", cutoff, 1, 1024000000, SCAP_TCP_FAST, 0);
	if (sc==NULL) {
...
void terminate(int signo) {
	...
	if ( scap_get_stats(sc, &stats) < 0 ) {
		...
		printf("pkts discarded: %lf %%\n",((double)stats.discarded_pkts/(double)stats.pkts)*100);
		printf("bytes discarded: %lf %%\n",((double)stats.discarded_bytes/(double)stats.bytes)*100);
	}
